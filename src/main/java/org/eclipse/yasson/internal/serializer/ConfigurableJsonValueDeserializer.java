...
public class ConfigurableJsonValueDeserializer extends AbstractValueTypeDeserializer<JsonValue> {
    ...
    public JsonValue deserialize(JsonParser jsonReader, DeserializationContext ctx, Type rtType) {
        final JsonParser.Event upcomingEvent = ((JsonbRiParser) jsonReader).getLastEvent();
        switch (upcomingEvent) {
            ...
            default:
                throw new JsonbException(Messages.getMessage(MessageKeys.INTERNAL_ERROR, "Unknown JSON value: "+ upcomingEvent));
        }
    }
