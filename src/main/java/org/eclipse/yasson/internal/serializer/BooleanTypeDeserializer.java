...
public class BooleanTypeDeserializer extends AbstractValueTypeDeserializer<Boolean> {
    ...
    public Boolean deserialize(JsonParser parser, DeserializationContext ctx, Type rtType) {
        JsonParser.Event event = ((JsonbStructureNavigator) parser).moveToValue();
        switch (event) {
            case VALUE_TRUE:
                return Boolean.TRUE;
            case VALUE_FALSE:
            ...
            default:
                throw new JsonbException(MessageBundle.getMessage(MessageKey.INTERNAL_ERROR, "Unknown JSON value: " + event));
        }
    }
