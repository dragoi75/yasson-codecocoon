...
public class ObjectSerializer<T> extends AbstractContainerSerializer<T> {
     ...
     */
    public ObjectSerializer(SerializationBuilder builder) {
        super(builder);
    }
     ...
     */
    public ObjectSerializer(CurrentItemWrapper<?> wrapper, Type runtimeType, ClassDescriptor classModel) {
    ...
    protected void serializeInternal(T object, JsonGenerator generator, SerializationContext ctx) {
        final PropertyModel[] allProperties = ((JsonbMarshaller) ctx).getMappingContext().getOrCreateClassModel(object.getClass()).getSortedProperties();
    ...
    private void marshallProperty(T object, JsonGenerator generator, SerializationContext ctx, PropertyModel propertyModel) {
        ...
        if (propertyModel.isReadable()) {
            ...
            Optional<Type> runtimeTypeOptional = ReflectiveTypeUtils.resolveTypeOptional(this, propertyModel.getPropertyType());
            Type genericType = runtimeTypeOptional.orElse(null);
            final JsonbSerializer<?> serializer = new SerializationBuilder(marshaller.getJsonbContext()).setWrapper(this).setObjectClass(propertyValue.getClass()).setCustomization(propertyModel.getCustomization()).setType(genericType).buildSerializer();
