...
public class OptionalValueSerializer<T extends Optional<?>> implements CurrentItemWrapper<T>, JsonbSerializer<T> {
    ...
    public void serialize(T value, JsonGenerator jsonWriter, SerializationContext serializationContext) {
        ...
        final JsonbSerializer<?> valueAdapter = new SerializationBuilder(runtimeContext).setObjectClass(presentValue.getClass())
                .setType(elementType).setWrapper(currentItemRef).setCustomization(serializationConfig).buildSerializer();
