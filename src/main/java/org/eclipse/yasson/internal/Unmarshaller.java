...
public class Unmarshaller extends ProcessingContext implements DeserializationContext {
    ...
    private <T> T deserializeItem(Type type, JsonParser parser) {
        try {
            DeserializerBuilder deserializerBuilder = new DeserializerBuilder(jsonbContext).withType(type).withJsonValueType(getRootEvent(parser));
     ...
     */
    public Unmarshaller(JsonbBindingContext jsonbContext) {
        super(jsonbContext);
    }
