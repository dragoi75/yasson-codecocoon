...
public class DefaultNamingStrategyProvider {
     ...
     */
    private static final Map<String, PropertyNamingStrategy> PROPERTY_NAMING_POLICIES;
    ...
    static {
        ...
        collector.put(PropertyNamingStrategy.IDENTITY, new IdentityNamingStrategy());
        collector.put(PropertyNamingStrategy.CASE_INSENSITIVE, new CaseInsensitiveStrategy());
        PROPERTY_NAMING_POLICIES = Collections.unmodifiableMap(collector);
    }

    /**
     * Gets naming conversion strategy by name, see {@link javax.json.bind.config.PropertyNamingStrategy}
     *
     * @param strategyKey name not null
     * @return strategy to use for conversion
     */
    public static PropertyNamingStrategy getStrategy(String strategyKey) {
        return PROPERTY_NAMING_POLICIES.get(strategyKey);
    }
}
