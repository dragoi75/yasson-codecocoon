...
class ClassParser {
        ...
        for (PropertyModel collectedPropertyModel : collectedProperties) {
            for (PropertyModel checkedPropertyModel : checkedProperties) {
                if ((checkedPropertyModel.getReadName().equals(collectedPropertyModel.getReadName()) && checkedPropertyModel.isReadable() && collectedPropertyModel.isReadable()) || (checkedPropertyModel.getWriteName().equals(collectedPropertyModel.getWriteName())) && checkedPropertyModel.isWritable() && collectedPropertyModel.isWritable()) {
                    throw new JsonbException(ResourceBundleMessages.getMessage(MessageConstants.PROPERTY_NAME_CLASH, checkedPropertyModel.getPropertyName(), collectedPropertyModel.getPropertyName(), cls.getName()));
                }
            }
    ...
    public void parseProperties(ClassDescriptor classModel, JsonbAnnotatedElement<Class<?>> classElement) {
        ...
        sortedPropertyModels.addAll(jsonbContext.getConfigProperties().getPropertyOrdering().sortPropertiesByOrder(classPropertyModels, classModel));
