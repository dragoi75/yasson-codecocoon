...
public class ComponentMatcher {
    ...
    private <T extends AbstractComponentBinding> Optional<T> searchComponentBinding(Type runtimeType, ComponentSupplier<T> supplier) {
        for (ComponentBindings componentBindings : userComponents.values()) {
            ...
            if (null != component && matches(runtimeType, componentBindings.getBindingType())) {
    ...
    private boolean matches(Type runtimeType, Type componentBindingType) {
        ...
        return runtimeType instanceof ParameterizedType && componentBindingType instanceof ParameterizedType && ReflectionTypeResolver.getRawType(componentBindingType).isAssignableFrom(ReflectionTypeResolver.getRawType(runtimeType)) && matchTypeArguments((ParameterizedType) runtimeType, (ParameterizedType) componentBindingType);
    }
    ...
    AdapterBinding introspectAdapterBinding(Class<? extends JsonbAdapter> adapterClass, JsonbAdapter instance) {
        final ParameterizedType adapterRuntimeType = ReflectionTypeResolver.findParameterizedInterfaceType(adapterClass, JsonbAdapter.class);
    ...
    DeserializerBinding introspectDeserializerBinding(Class<? extends JsonbDeserializer> deserializerClass, JsonbDeserializer instance) {
        final ParameterizedType deserializerRuntimeType = ReflectionTypeResolver.findParameterizedInterfaceType(deserializerClass, JsonbDeserializer.class);
    ...
    SerializerBinding introspectSerializerBinding(Class<? extends JsonbSerializer> serializerClass, JsonbSerializer instance) {
        final ParameterizedType serializerRuntimeType = ReflectionTypeResolver.findParameterizedInterfaceType(serializerClass, JsonbSerializer.class);
    ...
    private Type resolveTypeArg(Type adapterTypeArg, Type adapterType) {
        if (!(adapterTypeArg instanceof ParameterizedType)) {
            if (!(adapterTypeArg instanceof TypeVariable)) {
                ...
                return ReflectionTypeResolver.resolveItemTypeVariable(new RuntimeTypeHolder(null, adapterType), (TypeVariable<?>) adapterTypeArg);
            }
        } else {
            return ReflectionTypeResolver.resolveTypeParameters((ParameterizedType) adapterTypeArg, adapterType);
        }
    }
}
