...
public class ReflectiveTypeResolver {
    ...
    public static Class<?> getRawType(Type target) {
        return getOptionalRawType(target)
                .orElseThrow(()->new JsonbException(Messages.getMessage(MessageKeyConstants.TYPE_RESOLUTION_ERROR, target)));
    }
    ...
    public static Type resolveActualType(RuntimeTypeInfo element, Type target) {
        ...
        } else if (target instanceof ParameterizedType && element != null) {
    ...
    public static Type resolveGenericTypeArguments(ParameterizedType targetParameterized, Type searchTarget) {
        ...
        return Arrays.equals(concreteArgs, pendingArgs) ?
                targetParameterized : new ResolvedParameterizedType(targetParameterized, concreteArgs);
    }
    ...
    public static ParameterizedType locateParameterizedType(Class<?> targetClass, Class<?> targetInterface) {
        ...
        while (clazzCursor != Object.class) {
            for (Type interfaceType : clazzCursor.getGenericInterfaces()) {
                if (interfaceType instanceof ParameterizedType &&
                        targetInterface.isAssignableFrom(
                                ReflectiveTypeResolver.getRawType(((ParameterizedType) interfaceType).getRawType()))) {
